...
- 보고서는 `초록 -> 서론 -> 방법론 -> 결과 -> 결론` 순서로 읽히게 작성합니다.
- 보고서는 먼저 왜 이 실험을 하는지, 그다음 방법론과 지표 정의, 이후 데이터 조건과 기초통계량, 마지막으로 그래프와 수치의 뜻을 해석하는 순서를 따릅니다.
- `test/experiment_specs/`에는 실험 설계 배경, 보고서 양식, 참고문헌 메모를 둡니다.
