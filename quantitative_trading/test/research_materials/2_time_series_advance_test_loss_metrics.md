...
### 6.6. 1차 차분 후에도 지속되는 비정상성과 Epoch 0-5 수직 하강(Loss Plunge) 현상 심층 분석

실험 결과, 가격 원본 종가 대신 **1차 차분(Difference)**을 적용했음에도 불구하고 거의 모든 딥러닝 모델의 훈련 곡선에서 **Epoch 0-5 사이에 손실(Loss)이 마치 벼랑 끝에서 떨어지듯 수직으로 급락하는 현상**이 여전히 관측됩니다. 

*   **수학적 원인 진단 (The Trivial Predictor & Random Walk Trap)**:
    1.  **극도로 낮은 신호 대 잡음비 (Low SNR)**: 금융 시계열의 1차 가격 차분값($d_t = P_t - P_{t-1}$)은 통계적으로 평균이 0에 수렴하고 방향성이 예측 불가능한 '화이트 노이즈(White Noise)' 또는 '무작위 워크(Random Walk)'의 증분과 유사합니다. 즉, 과거 종가들의 변동 역사만으로는 다음 15분 동안 가격이 오를지 내릴지 알아내기가 수학적으로 극도로 어렵습니다.
    2.  **MSE/MAE의 평균 회귀 성질**: 평균제곱오차(MSE)나 평균절대오차(MAE)는 예측 오차의 분산(Variance)을 최소화하도록 뉴런을 학습시킵니다. 예측 불가능한 화이트 노이즈 흐름 하에서 오차 분산을 줄이는 수학적 최적해(Optimal Predictor)는 복잡한 변동을 맞추는 것이 아니라, **단순히 타겟의 기댓값(Mean)인 "항상 0에 가까운 값"을 고정 출력하는 것**입니다.
    3.  **Trivial Predictor (평균 예측의 덫)로의 빠른 수렴**: 모델은 학습 시작 직후 1~3 에포크 만에 복잡한 시계열 구조를 완전히 포기하고, 단순히 **"상수 0"에 가까운 평탄한 예측값을 출력하도록 출력층의 바이어스(Bias)와 가중치를 수렴**시킵니다. 이 단순 치팅 행위만으로도 예측 오차는 즉시 '데이터의 변동성 분산' 수준으로 급감하게 되며, 이로 인해 Epoch 0-5 사이에 손실 곡선이 수직으로 하강하게 됩니다. 
    4.  **학습 진척 없는 겉보기 수렴**: 이 수직 하강은 모델이 실제 유용한 미래 패턴을 해독해서 생긴 것이 아니라, **아무것도 학습하지 않는 가장 안전한 바보 모델(Trivial Baseline)**로 빠르게 수렴해 주저앉은 상태를 증명합니다. 또한 1차 차분만을 수행할 경우, 자산 데이터 특유의 **변동성 군집(Volatility Clustering)**이나 **분포 전환(Regime Shift)**과 같은 고차원 비정상성(Residual Non-Stationarity)은 여전히 잔존하여 모델의 발목을 잡습니다.

*   **학술적/기술적 SOTA 해결 방안**:
    1.  **가역 인스턴스 정규화 (Reversible Instance Normalization - RevIN, ICLR 2022)**:
        - 입력 시퀀스 국소 구간의 평균과 편차를 제거하여 완벽한 정상성(Stationarity)을 부여한 채 백본 모델에 넘겨주고, 모델의 출력부에 다시 대칭적인 역정규화(Denormalization) 연산을 적용해 학습시킵니다. 이는 비정상성으로 인한 분포 전이(Distribution Shift) 문제를 차단하여 모델이 '0'으로 도피하는 현상을 물리적으로 억제합니다.
    2.  **Soft-DTW / Shape-Aware Loss 도입 (Cuturi et al., ICML 2017)**:
        - 점 단위 오차(MSE/MAE)의 극단적 취약점을 우회하기 위해 시간축 왜곡 정렬 비용 자체를 무한 미분 가능하게 설계한 **soft-DTW Loss**나, 본 프로젝트에서 설계한 **Direction-Aware Loss (DALC)**, 혹은 **포트폴리오 샤프 레이쇼(Sharpe Ratio) 극대화 손실 함수**를 단독 또는 융합 적용합니다. 이로써 모델이 평균값 근처에서 사리는 대신, 추세의 실제 '형상(Shape)'과 '방향(Direction)'을 재현하도록 그래디언트 흐름을 강력히 강제합니다.
    3.  **구조적 분해 아키텍처 도입 (DLinear / Autoformer)**:
        - 시계열을 장기 추세(Trend)와 계절성/잔차(Seasonal/Residual) 컴포넌트로 명시적으로 분해하여 각각 독립된 신경망으로 훈련시키는 아키텍처적 유도 편향(Inductive Bias)을 도입하여, 노이즈 속에 파묻힌 추세 성분을 물리적으로 보호합니다.
    4.  **다변량 고차 특징 피처(Exogenous Features) 보강**:
        - 단순히 과거 가격 차분 하나만 주는 단변량 방식에서 탈피하여, 거래소의 실시간 호가창 잔량(Order Book Imbalance), 다중 종목 간의 동적 DTW 거리 특징, 기술적 보조지표(RSI, MACD) 등을 멀티 채널로 입력하여 모델이 노이즈 속에서 실질적인 신호(Signal)를 추출할 수 있는 풍부한 단서(SNR 상승)를 제공합니다.

---

### 6.7. PyTorch GPU 연산과 CPU NumPy 변환 메커니즘 심층 탐구: .cpu() vs .detach().numpy()

머신러닝 파이프라인 연산 과정 중 GPU 텐서에서 NumPy 배열로 변환하는 시점의 하드웨어 메모리 아키텍처와 PyTorch 연산 그래프 자동 미분(Autograd) 메커니즘의 차이를 명확히 분석합니다.

*   **1. 물리적 하드웨어 메모리 격리 (VRAM vs System RAM)**:
    - **GPU(CUDA)**: AI 모델의 초고속 선형대수 행렬 연산은 전용 그래픽 메모리(VRAM) 위에서 동작합니다.
    - **CPU(Host RAM)**: NumPy, Matplotlib, Scikit-learn 등의 전통적인 Python 데이터 분석 도구들은 전적으로 일반 시스템 메모리(System RAM) 위에서 레거시 C코드로 구현되어 동작합니다.
    - 따라서, GPU VRAM 영역에 존재하는 데이터는 하드웨어 인터페이스(PCIe 버스)를 통해 물리적으로 **CPU RAM 영역으로 복사(DtoH - Device to Host)**해 와야만 NumPy가 메모리 주소(Pointer)를 읽어 제어할 수 있습니다. 이것이 바로 **`.cpu()`** 메서드의 본질적 역할입니다.

*   **2. 연산 그래프 역전파 링크 차단 (.detach())**:
    - PyTorch는 오차 역전파(Backpropagation)를 수행하기 위해 텐서 간의 연산 기록을 담은 **동적 계산 그래프(Dynamic Computation Graph)**를 실시간 메모리에 구축하며, 각 텐서는 `requires_grad=True` 속성을 가집니다.
    - 그래디언트 추적이 켜져 있는 텐서를 `.numpy()`로 다이렉트 변환하려고 시도하면, PyTorch는 **"학습용 미분 경로 정보(Grad)가 소실될 수 있다"**는 런타임 예외 경고를 내뿜으며 변환을 강제 거부합니다.
    - 따라서, 연산 그래프 추적을 안전하게 끊어내고 텐서를 단순 상수 데이터 상태로 해방해 주는 **`.detach()`** 메서드를 우선 호출해 주어야 에러가 나지 않습니다.

*   **3. 변환 메서드의 삼단계 흐름 및 가이드라인**:
    - **올바른 최적 시퀀스**: 항상 `[그래디언트 분리] ➡️ [하드웨어 이관] ➡️ [포인터 포맷 캐스팅]`의 엄격한 순서를 따라야 안전합니다.
      ```python
      preds = model(x_tensor).detach().cpu().numpy()
      ```
    - **상황별 최적 사용 지침**:
      1.  **`with torch.no_grad():` 보호 블록 내부**:
          - 이 스코프 안에서는 미분 트래킹이 자동 정지되므로 이론상 `.detach()`를 생략하고 바로 `tensor.cpu().numpy()`만 써도 통과됩니다. 그러나 코드 유지보수 중 스코프 외부로 이탈하여 에러가 날 가능성을 대비해 **항상 `.detach().cpu().numpy()`를 세트로 붙여 쓰는 것이 업계 최고 표준(Best Practice)**입니다.
      2.  **학습/역전파 영역 또는 스코프 외부**:
          - 무조건 `.detach().cpu().numpy()`를 선언해야 합니다. 순서가 어긋나거나 `.detach()`가 누락되면 파이썬 프로세스가 즉각 중단(Error crash)됩니다.
      3.  **메모리 복사 최적화**:
          - `.numpy()`는 메모리 복제(Clone)를 하지 않고 기존 버퍼의 물리 메모리 주소를 그대로 캐스팅하여 포인터만 던져주므로 극도로 빠릅니다. 단, NumPy 배열을 생성한 뒤 값을 수동 조작하면 CPU 내의 원래 텐서 값도 같이 변하므로, 안전한 격리가 필요할 땐 `.detach().cpu().clone().numpy()`를 사용하는 것이 권장됩니다.
