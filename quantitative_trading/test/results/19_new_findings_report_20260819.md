...
> **B 갈래의 기본 모델은 target horizon에 따라 갈린다.**
> - **target 4시간(h=16) → GARCH-t** (주기 제거 후). 단기에서는 최근 충격을 빠르게 반영하는
>   조건부 분산 모델이 유리하다.
> - **target 16시간(h=64) → HAR-RV(일·주·월)**. 먼 지평에서는 장기기억을 담는 구조가 이긴다.
> - **교차점이 4~16시간 사이에 있다.** 즉 목표 horizon을 먼저 확정하지 않으면 모델을 정할 수 없다.
