...
작성일: 2026-07-23
관련 커밋: STEP1 `d294b18`, STEP2 (이 문서와 함께)
...
**평가 질문**: 과거 유사국면(이웃)의 *이후* 전개가 현재 국면의 *이후* 전개를 예측하는가?
연구 목적(방향=무신호, 크기/변동성=유신호, MDD=생존제약)에 맞춰 세 축을 보고, **랜덤 이웃
baseline**과 대조해 "유사도 검색이 실제로 정보를 더하는지"를 판정한다.
...
- [ ] context 채널(외생 텍스트/감성)이 채워지면 자동으로 가중치에 복귀(현재는 0이라 드롭 중).
