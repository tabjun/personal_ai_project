...
✓ 유효(10%+) · △ 주의(2~10%) · ✗ 무효(2% 미만). \* **분리도는 R² 값 자체가 아니다** — `|실모델−무모델| / max(|둘|)`이며, 둘 다 0 근처에서 부호가 다르면 100%를 넘을 수 있다 (B상황 실측: 무모델 R²=−0.000055, 실모델 R²=+0.000461, 분리도=112%. R² 자체는 항상 1 이하다).

### 9.2 이 표에서 읽어야 할 것 세 가지

**첫째 — 문제는 R²가 아니라 축이었다.** 레벨축에서는 **11개 지표 전부가 무효**다(0.0~0.2%).
반면 차분축에서는 **R²가 가장 민감한 분리자**다(분리도 112% — 이 112%는 R² 값이 아니라 무모델·실모델을 구별하는 정도를 재는 별도 지표다. R² 실제 값은 무모델 −0.000055, 실모델 +0.000461로 둘 다 0 근처다).
...
| # | v1 서술 | 정정 |
| ---: | :--- | :--- |
| 1 | "R²는 유효하지 않다" | **레벨축에서 무효, 차분축에서는 가장 민감한 분리자.** 축이 먼저다. (참고: '분리도 112%'는 R² 값이 아니라 구별력 지표 — R² 자체는 −0.000055 vs +0.000461로 둘 다 0 근처) ([9절](#9)) |
