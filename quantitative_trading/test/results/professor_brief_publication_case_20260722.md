...
작성일: 2026-07-22 · 브랜치: `stock`

## 요약 (Summary)

- **무엇을 했나**: 업비트 원화(KRW) 마켓 암호화폐 15분봉으로 비정상 시계열의 추세를 딥러닝으로
  예측하는 연구(실험 1~17번). 이 연구의 최종 목표는 예측 정확도 자체가 아니라, **과거 유사
  국면·시장 텍스트·예측/위험 신호를 LLM에 태워 상황을 설명·자문하는 시스템**의 '예측 재료'를
  만들고 그 한계를 정직히 규명하는 것.
- **핵심 발견 (데이터로 확정)**: 15분봉에서 **방향(다음이 오를지/내릴지)은 사실상 예측 불가**
  (자기상관 ≈ 0.05, 방향 정확도 0.48~0.54 = 동전 던지기). 반면 **변동의 '크기'(변동성)는 예측
  가능**(자기상관 0.36, GARCH로 0.46). 이는 우리 모델의 결함이 아니라 효율시장 하 고빈도
  수익률의 알려진 구조이며 문헌과 일치.
- **논문 각도**: 성능이 아니라 기여로 성립. ① 고빈도 crypto 방향 예측의 한계를 전종목·다모델·
  다손실로 체계 실증한 negative result, ② 두 붕괴 모드(진폭 압축·분산 폭주)의 진단과 교정
  (RevIN·분위 손실), ③ 평가 프레임의 자기기만(변동 없는 예측이 방어 우수로 오인되는 MDD 단독
  지표) 문서화.
- **결정이 필요한 지점**: 예측 대상(target)/차분 방식에 **정답이 하나가 아니라 갈래가 여럿**
  (A 방향 / B 변동성 / C 추세). 이론적 선험 답이 없어 실험 + 최종 LLM 필요에 따라 골라야 함.
  신호가 실재하는 **B(변동성)를 주 재료로 승격**하는 방향을 제안.
- **여쭐 것**: (1) negative-result + 진단/교정을 방법론 논문으로 정리하는 방향이 적절한지,
  (2) 예측 대상을 변동성(B) 중심으로 갈지, (3) 확장(GARCH 베이스라인·외생정보·cross-sectional·
  최종 LLM 결합)을 본 논문에 넣을지 후속으로 뺄지.
...
## 0. 최종 목표와 이 연구의 위치 (큰 그림)

이 프로젝트의 **최종 산출물은 "LLM 기반 상황 자문 시스템"**입니다: 현재 국면을 (a) 과거 유사
국면(DTW), (b) 당시·현재 시장 텍스트/감성, (c) 예측·위험 신호와 함께 LLM에 태워 "지금 왜
이런 상황이고, 과거 유사 사례에서 반등/붕괴 트리거는 무엇이었으며, 방어적으로 어떻게
대응할지"를 설명·판단하게 하는 것입니다(MDD 방어를 생존 제약으로). 지금까지 `test/`에서 한
1~17번 예측 연구는 **그 시스템에 들어갈 '예측/위험 신호' 재료를 만들고 그 한계를 규명하는
단계**입니다. 즉 예측 정확도 자체가 최종 목표가 아니라, LLM에 넘길 재료의 품질과 정직한
한계를 아는 것이 목적입니다.
...
## 7. 결정이 필요한 지점 — 예측 대상(target)/차분 방식

EDA로 드러난 사실: **비정상 시계열을 다루는 방식(target/차분)에 정답이 하나가 아니라 갈래가
여럿이고, 각각 트레이드오프가 다릅니다.** 어느 쪽이 "더 낫다"는 이론적 선험 답이 없고 —
데이터로 실험해봐야 하며, 게다가 **최종 LLM 시스템이 어떤 재료를 필요로 하느냐**에도 달려
있습니다. 그래서 이건 진행 전에 결정이 필요한 지점입니다.

| 갈래 | 예측 대상 | 차분 | 신호 유무(EDA) | LLM에 줄 재료로서 |
|---|---|---|---|---|
| A. 방향(부호) | 다음 수익률 상승/하락 | 차분함(수익률) | **거의 없음**(자기상관≈0.05) | 약함 — 단독으론 부적합 |
| B. 변동성(크기) | 앞으로의 변동 폭 | 차분함(수익률) | **있음**(자기상관 0.36, GARCH 상관 0.46) | 강함 — "지금 위험 국면인가" |
| C. 추세/레벨 | 가격 추세 자체 | **차분 안 함**(RevIN) | 레벨은 예측되나 lag-copy 착시 위험 | 중간 — 국면 맥락 |

- **판단 근거**: A는 EDA·문헌 모두 한계 확정. B는 신호가 실제로 있고(실측 0.46) GARCH 같은
  고전 통계모델과 딥러닝을 결합할 수 있음. C는 사용자가 원한 "차분 없이 비정상 직접"이나
  평가를 엄격히(persistence 대비) 해야 착시를 피함.
- **여쭐 것**: 예측 재료를 **B(변동성) 중심**으로 갈지, C(추세)도 병행할지, A는 외생정보(뉴스·
  온체인)를 넣어야만 의미가 생기므로 그 데이터 수집을 지금 시작할지. 이 선택이 남은 실험과
  최종 LLM 입력 설계를 좌우합니다.

## 8. 교수님께 여쭐 논의점 (종합)

1. 위 negative-result + 진단/교정을 **방법론 논문**으로 정리하는 방향이 적절한지.
2. 예측 대상(7절 A/B/C)을 무엇 중심으로 갈지 — 특히 신호가 실재하는 **변동성(B)**을 주 재료로
   승격하는 방향에 대한 의견.
3. 다음 확장(변동성 GARCH 베이스라인 vs 딥러닝 / 외생정보 도입 / cross-sectional / 최종 LLM
   결합)을 본 논문에 포함할지, 후속으로 분리할지.
4. 평가 프레임(방향+크기+생존제약 3축)과 baseline 선정이 심사에서 방어 가능한지.
