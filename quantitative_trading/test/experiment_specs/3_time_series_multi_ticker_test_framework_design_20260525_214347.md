...
> **수행 대상**: 업비트 원화(KRW) 마켓 전수 조사 (약 115개 종목 코호트)  
...
본 3단계 고도화 실험은 단일 가상자산 종목(BTC)의 지엽적인 시계열 변동성 학습 한계를 돌파하고, **업비트 원화(KRW) 마켓 전체 종목의 유니버스(Universe)를 전수 순회하는 고속 다중 종목 시계열 예측 아키텍처**를 구축하는 데 목적이 있습니다. 

특히, 업비트 전체에 상장된 약 250여 개 거래 페어(BTC, USDT 마켓 포함) 중 **오직 원화(KRW) 마켓 종목군(약 115개)만을 전수 조사 대상으로 최종 한정**하며, 이 설계 결정의 학술적/실전적 타당성은 다음과 같습니다:

1. **원화(KRW) 기준 화폐 단위 통일 (KRW Currency Unification)**:
   - 교수님의 기초 자료인 `materials\research_overview.docx`에 수록된 연구 지침에 부합하기 위해, 모든 포트폴리오 평가 및 모의투자 시뮬레이션의 계산 단위를 **대한민국 원화(KRW)로 100% 통일**합니다. 서로 다른 기준 화폐(BTC, USDT 등)가 혼재될 경우, 절대 가치 비교 및 자금 운용 시나리오 설계가 불가능해집니다.
2. **이중 변동성(Dual Volatility) 리스크 원천 배제**:
   - BTC 마켓 등에서 타 자산을 거래할 경우, 해당 알트코인의 가치 변동뿐만 아니라 **비트코인 자체의 원화 대비 가치 등락(기초 자산 변동성)이 이중으로 누적 반영**되어 퀀트 모델의 오차 분석을 왜곡시킵니다. 따라서 기저 화폐 변동성이 완전히 제거된 KRW 다이렉트 페어만 타겟팅합니다.
3. **슬리피지(Slippage) 제어 및 유동성 확보**:
   - 업비트 전체 거래 유동성의 90% 이상이 KRW 마켓에 집중되어 있으며, 호가창이 매우 두껍습니다. BTC/USDT 마켓의 극심한 슬리피지(체결 오차 가격) 비용을 억제하여 퀀트 모델의 실전 백테스팅 신뢰성을 극대화합니다.

본 분석에 사용되는 기초 원천 데이터셋은 **15분 단위의 종목별 고빈도 가격 데이터 (15-Minute Candle Data / 15분봉)**입니다. 15분봉 데이터가 지닌 조밀하고 불규칙한 초단기 변동성 노이즈 속에서, 교수님의 `research_overview.docx`에 수록된 "자금 관리 및 변동성 폭발 흐름 포착" 퀀트 사상을 신경망 학습 파이프라인과 긴밀하게 연계하여, 시장 전체의 유동성 흐름과 종목별 예측 정합성 편차를 한 장의 정밀한 주식 차트로 입증하고자 합니다.
