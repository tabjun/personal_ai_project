...
🎯 파일의 역할 및 생성 이유:
    본 스크립트는 복잡한 모델을 배제하고 핵심적인 최신 SOTA 시계열 아키텍처 모델군을 직관적으로 학습하고 분석하기 위해 작성된 [코어 7종 모델 노트북 생성기]입니다.
    - **생성 이유**: 15종 전수 조사 전단계 또는 경량화 검증 단계에서, 핵심적인 7개 핵심 모델(Mamba, mTAND 등 SOTA 중심)에 대해서만 정밀 튜닝 및 가이드 문서를 제공하여 개발 생산성을 높이기 위함입니다.
    - **수행 기능**: Keras와 PyTorch의 개발 패러다임 차이(Sequential vs OOP nn.Module)를 설명하는 튜토리얼 마크다운과 함께, 핵심 7종 모델 훈련, DTW 거리 계산, 시각화 저장을 완전하게 세팅한 주피터 노트북(`test/models/2_time_series_advance_test.ipynb`)을 자동 생성합니다.
"""
import json
import os
