...
🎯 파일의 역할 및 생성 이유:
    본 스크립트는 시계열 분석을 위한 노트북 코드가 스파게티 형태로 어지러워지는 것을 방지하고, 학교 서버 등 분산 환경에서의 유지보수 및 자원 격리를 극대화하기 위해 작성된 [OOP 기반 노트북 구조화(Reconstruction) 도구]입니다.
    - **생성 이유**: 절차 지향적으로 나열된 대용량 주피터 노트북은 재사용과 리스크 대응이 불가능합니다. 이를 완벽하게 결합도(Coupling)를 낮춘 모듈형 아키텍처로 강제 교정하기 위함입니다.
    - **수행 기능**: 노트북 내부의 코드를 `DataManager`(데이터 관리 객체), `ModelZoo`(알고리즘 팩토리 객체), `ResultSaver`(결과 및 이미지 저장소 객체)와 같은 독립적인 객체 지향형 클래스 구조로 재구성하여, 주피터 노트북(`test/models/2_time_series_advance_test.ipynb`)을 완전히 리팩토링 및 덮어씌웁니다.
"""
import json
import os
