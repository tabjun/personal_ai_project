...
🎯 파일의 역할 및 생성 이유:
    본 스크립트는 15종 이상의 모든 시계열 모델을 한눈에 체계적으로 전수 비교 평가하기 위해 작성된 [전수 조사(Exhaustive) 노트북 생성기]입니다.
    - **생성 이유**: 수많은 딥러닝 아키텍처(RNN, TCN, Attention 계열 등 15종)를 손수 코드로 붙여넣어 노트북을 만드는 번거로움을 해결하고, 완벽하게 통일된 입출력 인터페이스를 유지하며 훈련 루프를 일괄 자동화하기 위함입니다.
    - **수행 기능**: 15종 모델 클래스 정의, 데이터 전처리 파이프라인, 고해상도(DPI 300) 5대 이미지 생성 및 학습 통계치 메타데이터(`metadata.json`) 추출 코드가 내장된 `test/models/2_time_series_advance_test.ipynb` 전수 조사 특화 노트북을 자동 생성합니다.
"""
import json
import os
...
# --- Markdown: Intro ---
add_md("# 📊 시계열 가격 예측 알고리즘 전수 분석\n분석 목적: 15종 이상의 시계열 모델을 3년치 15분봉 데이터를 기반으로 전수 조사합니다.")

# --- Code: Setup ---
add_code(r"""# [1] 라이브러리 로드 및 환경 설정
import pandas as pd
import numpy as np
import torch
...
# --- Code: Data Loading ---
add_code(r"""# [2] DuckDB 접속 및 3년치 데이터 강제 확보
...
# --- Code: Viz ---
add_code(r"""# [5] 심층 시각화 (DPI 300 고해상도 인라인 플롯 구성)
import os
