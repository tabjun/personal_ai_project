...
def run_database_simulation_and_generate_report_v2():
    ...
    # Antigravity 인지 엔진의 차트 및 캔들 패턴 판독 기록
    cognitive_notes = {
        ...
        10: "[Antigravity 차트 판독]: 솔라나(SOL)가 최근 5개 캔들의 고가를 장대 양봉으로 관통하며 강력한 돌파(Breakout) 패턴을 형성 중입니다. 캔들의 아랫꼬리가 없고 몸통이 위로 꽉 찬 'Bullish Marubozu(장대양봉)'가 출현하였으며, 이는 매수세의 모멘텀이 극대화되었음을 강력히 지시합니다. 단기 차트 엣지가 살아있으므로 모의 투자를 개시합니다.",
        ...
        14: "[Antigravity 차트 판독]: 솔라나의 15분 차트 고점에서 긴 윗꼬리를 가진 비석형 'Gravestone Doji' 및 'Shooting Star(유성선)' 캔들이 포착되었습니다. 고점에서의 강력한 저항 매물이 출현하여 추세 반전의 리스크가 급증하였습니다. 이에 따라 손절 매도 감시벽의 실시간 트리거를 강하게 모니터링합니다.",
        15: "[Antigravity Risk Manager 작동]: 시장 전반에 급격한 풀백 매물이 쏟아지며 솔라나 가격이 장중 최저점인 270,275 KRW 이하로 급락하였습니다. Antigravity의 인지 판단에 따라, 추가 하방 압력을 차단하기 위해 매수가 대비 정확히 -2.0% 지점(슬리피지 포함 최종 -2.15% 손실선)에서 **기계적 손절(Stop-Loss)**을 일괄 집행하여 최대 하방 리스크를 원천 봉쇄하였습니다.",
        16: "[Antigravity 차트 판독]: 비트코인이 114,600,000 KRW의 강력한 매도 벽을 강한 거래량과 함께 관통하는 양봉이 형성되었습니다. 직전 음봉의 몸통을 완전히 덮어씌우는 정통 **'Bullish Engulfing (상승 장악형)'** 패턴이 출현하였습니다. 동시에 SMA-5선이 SMA-20선을 돌파하는 골든크로스가 발생하여, 비트코인 및 동조 자산인 이더리움(ETH)의 동시 분할 진입을 확정하였습니다.",
        17: "비트코인과 이더리움 포지션 매수가 완료되었습니다. 진입 즉시 0.07% 수준의 슬리피지/수수료 패널티가 자산 평가액에 정확히 누적 반영되었습니다. 리스크 제어를 위해 현금 버퍼를 50% 수준으로 넉넉하게 고수합니다.",
        20: "비트코인이 114,630,000 KRW 선에서 상승 깃발형(Bullish Pennant) 패턴을 그리며 숨고르기 중입니다. 이평선의 지지력(SMA-20)이 무너지지 않는 한, Antigravity 인지 엔진은 추세 추종(Hold) 의견을 유지합니다.",
        24: "[Antigravity 차트 판독]: 시뮬레이션 종료 시점을 앞두고 비트코인 캔들이 고점에서 윗꼬리가 길어지는 행잉맨(Hanging Man) 패턴을 그리고 있으며, 단기 모멘텀이 둔화되고 있습니다. 현실적인 퀀트 운용 원칙에 입각하여 가상 자본의 최종 이익 확정을 위해 비트코인과 이더리움 잔존 포지션을 전량 청산(Liquidate)하고 자산을 현금으로 완전 귀환시킵니다."
    }
    ...
    # 이평선 및 Antigravity 인지 관찰 로그 테이블 생성
...
2. **Antigravity 내장 인지 모델의 실시간 캔들 차트 분석**:
   * 본 시뮬레이션의 의사결정은 정적 파이썬 룰에만 의존하지 않고, **Antigravity 내장 LLM이 15분봉 캔들의 크기, 꼬리 길이, 거래량 및 이동평균선의 각도를 직접 읽고 'Doji', 'Bullish Marubozu', 'Bullish Engulfing', 'Shooting Star' 등의 금융 캔들 패턴을 판독하여 거래를 집행**한 인지 동학을 그대로 담아내었습니다.
...
### 1.3 데이터에 근거한 실시간 기술 지표 및 Antigravity 인지 엔진 추적 내역
...
| 시간 (Time) | BTC 종가 (Price) | 단기 이평 (SMA-5) | 중기 이평 (SMA-20) | 국소 저항 (Resistance) | 거래량 (Volume) | Antigravity 인지 차트 리포트 및 액션 | 평가 자산 (Portfolio) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
{indicator_table_content}

* *인덕티브 바이어스 고찰: Antigravity AI는 매 스텝마다 가격뿐만 아니라 캔들의 기하학적 형태(도지, 유성선, 상승장악형)를 독립 인지하여 트레이딩 가이드라인을 주도했습니다.*
