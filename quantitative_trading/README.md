...
builder = TextFeatureBuilder(db_path="upbit_data.db")
...
- Core modules live under `contexts/`, `marts/`, and `pipelines/` so the project root stays focused on `main.py`, docs, and top-level assets.
...
mart = HistoricalFlowMart(HistoricalFlowConfig(db_path="upbit_data.db"))
