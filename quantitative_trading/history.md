...
| 2026-06-08 | Execution Actor Boundary Clarification | Corrected the project execution rule so repo-level automation remains preserved while Codex local execution is restricted. | Updated `AGENTS.md`, `skills.md`, and `conversation_l2_cache.md` to distinguish runnable automation code/commands for other researchers, school server, CI, and schedulers from Codex's no-local-analysis boundary in the user's personal research session; clarified research `.ipynb` files must be mirrored to same-name `.py` files. | Keep automation scripts and run commands in repo; Codex should write/review/docs and mirror notebook code only unless local execution is explicitly authorized. |
