...
| 2026-06-08 | Literature Review / Independent Variables | Researched paper-backed independent variables for stock and crypto forecasting without running local training, backtests, or notebook result pipelines. | Created `test/research_materials/independent_variables_literature_review_20260608.md` with 5-step paper reviews and a concrete variable schema; committed/pushed `216175b` to `origin/stock`; sent the GitHub-rendered report link to the professor with UTF-8 Korean email. | Use the variable schema as the next implementation target when extending the DuckDB feature mart. |
| 2026-06-08 | Historical Flow Data Mart | Designed and implemented a KRW-wide historical analog mart so past similar events are matched by price shape, independent-variable state, and context/causal factors rather than chart shape alone. | Added `historical_flow_mart.py`, `build_historical_flow_mart.py`, `query_historical_flows.py`, `test/research_materials/historical_flow_datamart_research_20260608.md`, and UTF-8 mail script; py_compile and tiny synthetic KRW multi-ticker build/query passed; committed/pushed `699e65e` to `origin/stock`; sent professor email. Heavy full-market build was not run locally. | Execute full mart build later on the school server or automation environment, then connect query results to reports/backtests. |
