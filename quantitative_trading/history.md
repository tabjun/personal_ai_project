...
| 2026-06-08 | Historical Flow Data Mart | Designed and implemented a KRW-wide historical analog mart so past similar events are matched by price shape, independent-variable state, and context/causal factors rather than chart shape alone. | Added `historical_flow_mart.py`, `build_historical_flow_mart.py`, `query_historical_flows.py`, `test/research_materials/historical_flow_datamart_research_20260608.md`, and UTF-8 mail script; py_compile and tiny synthetic KRW multi-ticker build/query passed. Heavy full-market build was not run locally. | Commit/push only `stock`, send professor email, then execute full mart build later on the school server or automation environment. |
