...
### 2.12 연구 방향 가드레일과 세션 보고 방식 (2026-07-18 사용자 지시)
...
- **당장 할 일**: 열려 있는 VSCode output이 아직 남아 있으면 leaderboard, feature group average,
  ...
  후속 실행을 준비한다.
- **핵심 기조 (2026-07-18 사용자 교정)**: 본선은 **변동을 제대로 재현·예측하는 모델**이다
  (변동폭 재현 + 추세 추종). 잃지 않기(MDD)는 기본 제약이지 랭킹 지표가 아니다 —
  **변동 없는(평탄한) 예측 위에서 측정된 MDD 개선은 성과로 인정하지 않는다.**
  ...
  단독 프레임은 2026-06-24 10·11 종합 보고서에서 시작된 이탈로 판정되어 폐기 —
  근거: test/results/15_research_trajectory_audit_20260718.md)
