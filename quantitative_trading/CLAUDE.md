...
- **[최상위] 연구 목적이 모든 개별 지시보다 우선한다 (2026-07-19 명시)**:
  이 연구의 목적은 **"비정상 시계열이 예측 추세(큰 변동 포함)를 얼마나 잘 잡도록 학습시키는가"**
  다. MDD(잃지 않기)는 그 목적 아래의 **생존 제약**이다 — 추세 변동을 잘 따라가도 하방선이
  무너지면 전체가 무너지므로 항상 함께 지키되, 그것이 목적 자체를 대체하지 않는다.
...
- **당장 할 일**: 열려 있는 VSCode output이 아직 남아 있으면 leaderboard, feature group average,
  ...
  후속 실행을 준비한다.
- **핵심 기조 (2026-07-19 사용자 확정)**: 연구 목적 = **"비정상 시계열이 예측 추세(큰 변동
  포함)를 얼마나 잘 잡도록 학습시키는가"** — 이것이 모든 개별 지시·지표보다 항상 최우선이다.
  MDD(잃지 않기)는 그 아래의 **생존 제약**: 추세를 잘 따라가도 하방선이 무너지면 전체가
  무너지므로 항상 함께 지키되, 랭킹 지표나 목적 자체로 격상하지 않는다.
