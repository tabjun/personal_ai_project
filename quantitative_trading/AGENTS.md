...
### 2.9g 모든 보고서는 자기완결적이어야 한다 (2026-09-07 신설)

**배경**: 데이터 조건(종목·빈도·기간·분할·전처리)을 첫 보고서에만 쓰고 이후 생략했더니,
나중 보고서만 보면 "어떤 데이터를 어떻게 다뤄서 이 결과가 나왔는지" 파악이 안 되고 방향이
엇나갔다(사용자 지적 2026-09-07).

**규칙**: 분석·모델링 보고서는 매번 아래 **표준 헤더 6항목**을 서두에 싣는다. 생략 금지이며,
이전 보고서 참조로 대체하지 않는다(링크는 추가로만 단다).

```
1. 데이터 출처·형식 : DB·테이블명, 봉 간격(15분), 종목명 전체 나열(top20이면 20개 다 적는다)
2. 수집 기간·규모   : 시작~종료 일시, 총 봉 수, 결측·갭 현황
3. 학습/검증 분할   : train/validation 비율과 실제 구간(날짜), 시간순 분할 여부
4. 기본 전처리·가정 : 로그 변환 → 1차 차분(정상성 확보) 등 파이프라인과 그 근거 검정값
5. 기초통계량·EDA   : 평균·표준편차·왜도·초과첨도, 이전 EDA에서 확인된 데이터 특성
6. 이번 회차 변환   : 이번에 새로 적용한 처리, 그 이유, **변환 후 기초통계량·검정값 재확인**
```

- 6항목은 `test/scripts/report_header.py`의 `render_standard_header()`로 생성해 재사용한다.
- **가정 미충족 상태에서 성능을 보고할 때는 반드시 한계를 명시한다** — 어떤 가정이 왜
  깨졌는지, 그 결과 해당 성능 수치를 어디까지 신뢰할 수 있는지 함께 적는다. 가정을 맞추지
  못한 채 성능표만 제시하는 것은 통계적으로 무의미하다(근거: `assumption_diagnosis_report_20260907.md`).
