...
## 1.1. 테스트 환경 운영 원칙 (Test Environment Protocols)

`test/` 디렉토리 내에서 분석 및 모델 개발을 수행할 때 반드시 준수해야 하는 규칙입니다.

- **데이터 분할:** 3년치 데이터 분석 시 반드시 **2년 학습(Train) / 1년 테스트(Test)** 구조를 유지하며, `TimeSeriesSplit`을 활용해 시계열 교차 검증을 수행합니다.
- **모델 선정 가이드:**
  - **mTAND**: 불규칙하게 샘플링된 데이터나 노이즈가 많은 구간의 규칙성/불규칙성 모델링에 사용합니다.
  - **Mamba**: 선형 시간 복잡도로 장기 의존성을 파악해야 하는 대규모 시퀀스 학습에 사용합니다.
- **문서화 및 워크플로우:** 작업 전 `test/research_materials/`에 논문 정보를 정리하고, 분석은 `test/models/` 내 노트북에서 수행하며, 최종 리포트는 `test/results/`에 저장합니다. 세부 사항은 `test/skills.md`를 상시 참조하십시오.
