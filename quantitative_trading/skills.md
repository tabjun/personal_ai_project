...
이 레포지토리는 다른 연구원/운영자가 재현 실행할 수 있도록 자동화 코드, CLI 엔트리포인트, 실행 명령 예시를 보존합니다. 다만 Codex는 사용자의 개인 연구 세션에서 분석/학습/백테스트를 로컬 터미널로 직접 실행하지 않습니다. 실제 딥러닝 학습, 노트북 실행, 분석 리포트 산출용 실험은 학교 서버 커널, CI/스케줄러, 또는 사용자가 지정한 원격 실행 환경에서 수행합니다.

- **레포 보존 범위**: 자동화 실행 코드, `uv run ...` 명령 예시, n8n/Cron/CI/Docker/Kubernetes 연동 설계, 서버 커널 실행 절차.
- **Codex 수행 범위**: 코드 작성, 정적 코드 리뷰, 설계 검토, 서버 커널 실행 절차 문서화, 연구용 `.ipynb`와 동명 `.py` 미러 파일 작성/동기화.
- **Codex 금지 범위**: 개인 연구 세션의 로컬 터미널 또는 `.venv`에서 `uv run main.py`, `uv run analyzer.py`, 노트북 실행, 딥러닝 학습, 백테스트/분석 결과 생성용 실행.
